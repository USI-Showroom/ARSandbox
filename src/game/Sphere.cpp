...
Sphere::Sphere(const double minH, const double maxH)
:_life(0), _minH(minH), _maxH(maxH), _resurrected(false)
{ }

void Sphere::think(const UnitSquareMapping &mapping)
{
	if (!alive())
		resurrect(mapping);
...
void Sphere::newDirection(const UnitSquareMapping &mapping)
{
    if(_decisionTicks<=0)
    {
...
double Sphere::randRange(const double min, const double max)
{
    return (double(rand())/RAND_MAX)*(max-min)+min;
}

void Sphere::resurrect(const UnitSquareMapping &mapping)
{
